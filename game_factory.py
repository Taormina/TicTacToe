...
class Game():
	...
	def construct_board(self):
		...
		for move in moves:
			...
			x = move % 3
			if board[y][x]:
				print("Something is wrong with game history! Please make a new game.")
...
def prepare_game(player1, player2):
	...
	try:
		...
		abort(409, "Game ID {} does not exist!".format(game_id))
...
def get_game(game_id):
	...
	if game_row:
		return Game(game_id, game_row["player1"], game_row["player2"])
	else:
		abort(409, "Game ID {} does not exist!".format(game_id))
...
def make_move(game_id, position):
	if not isinstance(game_id, str):
		abort(409, "Invalid game id, you gave: {}".format(game_id))

	if not isinstance(position, int) or position not in range(9):
		abort(409, "Invalid position, should be 0 <= position <= 8, you gave: {}".format(position))
	...
	try:
		...
		abort(409, "Couldn't insert move {} into game {}".format(position, game_id))
